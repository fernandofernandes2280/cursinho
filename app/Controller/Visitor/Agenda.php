...
class Agenda extends Page{
	...
	private static function getAgendaItems($request, &$obPagination){
		$resultados = '';
	...
	//	var_dump($results); exit;
		...
		while ($obAtendimento = $results -> fetchObject(EntityAtendimento::class)) {
			...
			//View de agendas
			$resultados .= View::render('visitor/modules/agenda/item',[
...
//		}
		...
		//Conteúdo da Home
		$content = View::render('visitor/modules/agenda/index',[
				
				'itens' => self::getAgendaItems($request,$obPagination),
		...
		return parent::getPanel('Agenda', $content,'agenda', self::$hidden);
