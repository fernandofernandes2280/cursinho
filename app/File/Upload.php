...
class Upload{
	...
	//método responsável por mover o arquivo de upload
	public function uploadFotoAluno($dir, $overwrite = true,$matricula,$nome ){
		...
		if(!empty($files['fImage'])){
			...
			if(empty($file['type']) || !in_array($file['type'], $upload::isAllowed())  ){
				...
				$obUpload = new Upload($files['fImage']);
					...
					$sucesso = $obUpload->uploadFotoAluno(__DIR__.'/files/fotos',false,$obAluno->matricula,$obAluno->nome);
				...
				//			unlink($filename);}
						//salva o nome do arquivo no banco
							$obAluno->foto = $obUpload->getBaseName();
							$obAluno->Atualizar();
